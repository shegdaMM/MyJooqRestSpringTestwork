...
DROP TABLE IF EXISTS `patientregsys`.`PositionBuild` ;
...
CREATE UNIQUE INDEX `PositionBuildcol_name_UNIQUE` ON `patientregsys`.`PositionBuild` (`PositionBuildcol_name` ASC) VISIBLE;

DROP TABLE IF EXISTS `patientregsys`.`Doctor` ;
...
DROP TABLE IF EXISTS `patientregsys`.`status` ;
...
CREATE UNIQUE INDEX `statuscol_name_UNIQUE` ON `patientregsys`.`status` (`statuscol_name` ASC) VISIBLE;



DROP TABLE IF EXISTS `patientregsys`.`MedicalSession` ;
...
CREATE INDEX `fk_MedicalSession_Patient_idx` ON `patientregsys`.`MedicalSession` (`Patient_Patientcol_ID` ASC) VISIBLE;

CREATE INDEX `fk_MedicalSession_PositionBuild1_idx` ON `patientregsys`.`MedicalSession` (`PositionBuild_PositionBuildcol_Id` ASC) VISIBLE;

CREATE INDEX `fk_MedicalSession_Doctor1_idx` ON `patientregsys`.`MedicalSession` (`Doctor_Doctorcol_ID` ASC) VISIBLE;

CREATE INDEX `fk_MedicalSession_status1_idx` ON `patientregsys`.`MedicalSession` (`status_statuscol_ID` ASC) VISIBLE;
